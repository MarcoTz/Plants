...
def update_species(species_name:str, update_key:str, update_value:str) -> None: 
    ...
    new_species_info : dict[str,str] = {
            ...
            "watering_notes"            : ''.join(species_info['watering_notes']),
            "avg_watering_days"         : str(species_info['avg_watering_days']),
            "fertilizing_notes"         : ''.join(species_info['fertilizing_notes']),
            "avg_fertilizing_days"      : str(species_info['avg_fertilizing_days']),
            "pruning_notes"             : ''.join(species_info['pruning_notes']),
            "companions"                : ''.join(species_info['companions']),
            "additional_notes"          : ''.join(species_info['additional_notes'])
            }

    if update_key in append_fields:
        current_value : str = new_species_info[update_key].strip()
        current_value += ', '+update_value if current_value != '' else update_value
